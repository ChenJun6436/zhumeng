...
public class InspectionServiceImpl implements InspectionService {
    ...
    public ResponseVO findInspectionInfo(UserSearch uSearch) {
        List<InspectionListVo> list = inspectinMapper.findListByKeyWords(uSearch.getKey(),
                                                                         uSearch.getStart(),
                                                                         uSearch.getLength());
    ...
    public ResponseVO findInspectionUserInfo(UserSearch userSearch) {
        ResponseVO responseVO = new ResponseVO();
        List<InspectionUserVo> list = inspectinMapper.findInspectionUserInfo(userSearch.getKey(),
                                                                             userSearch.getId(),
                                                                             userSearch.getStart(),
                                                                             userSearch.getLength());
        if (list != null) {
            for (InspectionUserVo inspectionUserVo : list) {
                inspectionUserVo.setAddress(userService.getAddressById(inspectionUserVo.getId()));
            }
        }
        long total = inspectinMapper.countInspectionUserInfo(userSearch.getKey(),
                                                             userSearch.getId());
        responseVO.setCode(ConstantsUtil.RES_SUCCESS_CODE);
        responseVO.setMessage(ConstantsUtil.RES_SUCCESS_MESSAGE);
    ...
    public List<OptionVo> getUsers() {
        ...
        return inspectinMapper.getUsers();
    }
}
