...
public class AreaServiceImpl implements AreaService {
	...
	public AreaIdsVo findAreaIdsByDistrict(String district) {
		AreaIdsVo vo= new AreaIdsVo();
		if(StringUtils.isEmpty(district)){
			return vo;
		}
		//区县id
		Area dis = mapper.findDistrictByDistrict(district);
		if(dis != null){
			vo.setDistrictId(dis.getId().toString());
			//市区id
			vo.setCityId(dis.getParent().toString());
			//省id
			Area city = mapper.findById(dis.getParent());
			if(city != null){
				vo.setProvinceId(city.getParent().toString());
			}
		}
		return vo;
	}
	...
	public AreaIdsVo findAreaIdsByCity(String city) {
		...
		Area area = mapper.findAreaByCity(city);
		vo.setCityId(area.getId().toString());
		vo.setProvinceId(area.getParent().toString());
		return vo;
	}
