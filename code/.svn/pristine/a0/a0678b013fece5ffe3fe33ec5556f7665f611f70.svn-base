...
public class AndSlaughterhouseUserController {
  ...
  @Autowired
  private PigFileMapper pigFileDao;
  ...
  public ResponseVO add(AndSlaughterhouseUser vo) {
    ...
    String pigId = vo.getPigId();

    boolean existPig = pigFileService.existPigId(pigId);
    if (existPig) {
      responseVO.setCode("800");
      responseVO.setMessage("该猪ID不存在！");
    } else {
      ...
      if (pigFile != null) {
        ...
        if (pigStatus != 0) {
          responseVO.setCode("800");
          responseVO.setMessage("猪ID非正常状态，不能录入！");
          return responseVO;
        }
      }
    }
   ...
   * 用户是否已有屠宰场
   */
  @RequestMapping(value = "/sign/and_slaughter_house/exist_by_userid", method = RequestMethod.POST)
  @ResponseBody
  public ResponseVO existByUserId(@RequestParam(value = "userId", required = true) String userId) {
    ResponseVO responseVO = new ResponseVO();

    boolean flag = service.existByUserId(userId);
    if (flag) {
      responseVO.setCode("800");
      responseVO.setMessage("登录账号已存在屠宰场，不能新增！");
      return responseVO;
    }
