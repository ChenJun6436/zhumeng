...
public class AndSlaughterhouseUserServiceImpl implements AndSlaughterhouseUserService {
    ...
    public boolean add(AndSlaughterhouseUser vo) {
        try {
            ...
            pigFileService.updateStatus(vo.getPigId(), 3);
            // 插入流程表
    ...
    public boolean addUserSlaughter(AndSlaughterhouseUser vo) {
        try {
            ...
            Slaughterhouse slaughterhouse = slaughterhouseDao.findById(vo.getSlaughterhouseId());
            Integer number = slaughterhouse.getNumber();
            slaughterhouse.setNumber(number == null ? 1 : ++number);
            slaughterhouseDao.update(slaughterhouse);
            return dao.insert(vo) > 0;
        } catch (Exception e) {
            e.printStackTrace();
