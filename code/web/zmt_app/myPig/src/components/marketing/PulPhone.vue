  ...
  <div class="login-box">
    ...
    <div class="login-form">
      <div class="input_box">
        ...
        <input v-model='phone' class="input" type="text" name="" placeholder="输入手机号"  :disabled='haveLogin'>
...
export default {
  ...
  methods: {
    ...
    getCode() {
      ...
      if(_this.haveLogin){
        codeData.tel = JSON.parse(localStorage.getItem('userInfo')).tel
      }
        ...
        if(_this.phone == ''){
          ...
          if(_this.code){
            if(/^\d{6}$/.test(this.code)){
                ...
                if(_this.haveLogin){
                  saveData.tel = JSON.parse(localStorage.getItem('userInfo')).tel
                }
                server()._verifyCheckCode(saveData).then(function(res){
                  if(res.data.code === '0'){
                    _this.$msg('验证成功','middle')
                    setTimeout(function(){
                      _this.$router.push('/Main/BindPhone')
                    },1500)
                  }else {
                    _this.canSvae = true
                    _this.$msg(res.data.message)
  ...
  created() {
    ...
    if( JSON.parse(localStorage.getItem('userInfo')) ){
      this.haveLogin = true
      this.phone = this._uPhone(JSON.parse(localStorage.getItem('userInfo')).tel) 
    }
  },
  computed: {
    _num : function(){
