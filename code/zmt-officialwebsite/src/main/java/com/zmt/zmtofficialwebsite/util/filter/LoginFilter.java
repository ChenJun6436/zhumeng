...
public class LoginFilter implements Filter {
   ...
   * before the filter is asked to do any filtering work. <p> The web container cannot place the filter into service if
   * the init method either: <ul> <li>Throws a ServletException</li> <li>Does not return within a time period defined by
   * the web container</li> </ul>
   *
   ...
   * @param chain Provides access to the next filter in the chain for this filter to pass the request and response to
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    //登陆地址判断
    if (!path.equals("/user/logout") && !path.equals("/user/login") && !path.contains("/web/")) {
